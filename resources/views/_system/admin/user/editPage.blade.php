            ...
            <div class="col-lg-12">
                <div class="ibox">
                    ...
                    <div class="ibox-content">
                        <form action="{{ auto_url('system/user/edit_submit') }}" method="post" class="form-horizontal"
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10 col-md-10 col-lg-10">
                                    <input type="text" class="form-control" id="page-edit-form-username" name="username" value="{{ $adminUser->username }}" autocomplete="off" required readonly>
                                ...
                                <div class="col-sm-10 col-md-10 col-lg-10">
                                    <table class="footable table table-stripped toggle-arrow-tiny">
                                        ...
                                        @foreach($roles as $item)
                                            <tr>
                                                <td>{{ $item->name }}</td>
                                                <td><input type="checkbox" class="i-checks" name="role[]" value="{{ $item->role_id }}" @if(in_array($item->role_id, $adminUserRole)) checked @endif></td>
                                                <td></td>
                                            </tr>
                                            @if($item->users['pivot']['is_admin'])
                                                @foreach($item->children_role as $role)
                                                    <tr>
                                                        <td>{{ $role->name }}</td>
                                                        <td><input type="checkbox" class="i-checks" name="role[]" value="{{ $role->role_id }}" @if(in_array($role->role_id, $adminUserRole)) checked @endif></td>
                                                        <td><input type="checkbox" class="i-checks" name="role_admin[]" value="{{ $role->role_id }}" @if(in_array($role->role_id, $adminUserRoleAdmin)) checked @endif></td>
                                                    </tr>
                                                @endforeach
                                            @endif
                                        @endforeach
