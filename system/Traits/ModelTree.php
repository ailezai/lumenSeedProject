...
trait ModelTree
{
     ...
     * @var string
     */
    protected $parentColumn = 'parent_menu_id';
    ...
    public static function saveOrder($tree = [], $parentId = 0)
    {
        ...
        foreach ($tree as $branch) {
            $node = static::find($branch['menu_id']);

            $node->{$node->getParentColumn()} = $parentId;
            $node->{$node->getOrderColumn()} = static::$branchOrder[$branch['menu_id']];
            $node->save();

            if (isset($branch['children'])) {
                static::saveOrder($branch['children'], $branch['menu_id']);
            }
        }
    }
